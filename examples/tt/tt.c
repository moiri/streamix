 ...
 * FILE EDIT THE ORIGINAL STREAMIX-SOURCE FILE `tt.smx` INSTEAD!
 *
...
int main( void )
{
    ...
    SMX_NET_CREATE( 6, smx_tf, smx_tf );
    SMX_NET_INIT_TF( 6, 0, 1000000 );
    SMX_CHANNEL_CREATE( 0, 1, SMX_FIFO_D, x );
    SMX_CHANNEL_CREATE( 4, 1, SMX_D_FIFO, x );
    SMX_CONNECT_TF( 6, 0, 4, x );
    SMX_CONNECT( 0, 0, l, x, out );
    SMX_CONNECT( 1, 4, m, x, in );
    SMX_CHANNEL_CREATE( 1, 1, SMX_FIFO_D, x );
    SMX_CHANNEL_CREATE( 5, 1, SMX_D_FIFO, x );
    SMX_CONNECT_TF( 6, 1, 5, x );
    SMX_CONNECT( 1, 1, m, x, out );
    SMX_CONNECT( 2, 5, m, x, in );
    SMX_CHANNEL_CREATE( 2, 1, SMX_FIFO_D, x );
    SMX_CHANNEL_CREATE( 6, 1, SMX_D_FIFO, x );
    SMX_CONNECT_TF( 6, 2, 6, x );
    SMX_CONNECT( 2, 2, m, x, out );
    SMX_CONNECT( 3, 6, r, x, in );
    SMX_CHANNEL_CREATE( 3, 100, SMX_FIFO, profiler );
    SMX_CONNECT( 5, 3, smx_profiler, profiler, out );
    SMX_CONNECT( 4, 3, smx_mongo, data, in );
    SMX_NET_FINALIZE_TF( 6 );
    SMX_NET_FINALIZE_PROFILER( 5 );

    SMX_PROGRAM_INIT_RUN();

    SMX_NET_RUN( 6, smx_tf, 3 );
    SMX_NET_RUN( 0, l, 2 );
    SMX_NET_RUN( 1, m, 2 );
    SMX_NET_RUN( 2, m, 2 );
    SMX_NET_RUN( 3, r, 2 );
    ...
    SMX_CHANNEL_DESTROY( 4 );
    SMX_CHANNEL_DESTROY( 5 );
    SMX_CHANNEL_DESTROY( 6 );
    SMX_NET_DESTROY( 0 );
    SMX_NET_DESTROY( 1 );
    SMX_NET_DESTROY( 2 );
    SMX_NET_DESTROY( 3 );
